...
export const sendDashboardReport = async (
    reportTitle: string,
    ...
    // Add project owner
    const owner = project.team.find(m => m.isOwner || m.userId === project.ownerId);
    if (owner?.email) recipients.push(owner);
    
    // Add leadership roles
    const leadership = project.team.filter(m => m.isLeadership && m.email);
    recipients.push(...leadership);
...
This report was automatically generated by the Project Management Accelerator.
...
Project Management Accelerator
        ...
        try {
            ...
            console.log(`Dashboard report sent to: ${recipient.email}`);
        } catch (error) {
            console.error(`Failed to send dashboard report to ${recipient.email}:`, error);
        }
    }
};
