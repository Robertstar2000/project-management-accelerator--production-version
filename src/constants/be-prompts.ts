...
`;
    ...
    phase2: (name, discipline, context, mode = 'fullscale', scope = 'internal', teamSize = 'medium', complexity = 'typical') => {
        ...
        const complexityInstruction = COMPLEXITY_INSTRUCTIONS[complexity];
        if (mode === 'minimal') {
            return `You are a project manager. Your task is to generate a minimal **Resources & Skills List** for a project named "${name}" in the "${discipline}" discipline. Use the provided high-level project context for guidance.\n\n--- HIGH-LEVEL CONTEXT ---\n${context}\n-------------------------\n\n${teamSizeInstruction} ${complexityInstruction} ${subcontractorInstruction} The listed roles and tools must be real and standard for the discipline; do not invent names. Use professional jargon.\n\nCRITICAL FOR PARSING - MANDATORY FORMAT:\n1. Use EXACT headings: "## Required Roles", "## Required Software", "## Required Hardware", "## External Partners"\n2. Under "## Required Roles", format as: "- Role Name: Skill1, Skill2"\n3. Each role on its own line with "- " prefix\n4. Must have colon ":" after role name\n5. Provide 2-3 essential roles minimum\n6. Write "- None" only if truly not applicable`;
        }
        return `You are an expert project manager. Your task is to generate a comprehensive **Resources & Skills List** for a project named "${name}" in the "${discipline}" discipline. Use the provided high-level project context for guidance.\n\n--- HIGH-LEVEL CONTEXT ---\n${context}\n-------------------------\n\n${teamSizeInstruction} ${complexityInstruction} ${subcontractorInstruction} The names for all listed resources (skills, roles, software, hardware, partners) must be standard and commonly used in the '${discipline}' industry. Do not invent names. You must use professional, industry-standard terminology.\n\nCRITICAL FOR PARSING - MANDATORY FORMAT REQUIREMENTS:\n1. You MUST use these EXACT Markdown headings: "## Required Roles", "## Required Software", "## Required Hardware", "## External Partners"\n2. Under "## Required Roles", you MUST provide a bulleted list with EXACTLY this format:\n   - Role Name: Skill1, Skill2, Skill3\n   Example: "- Project Manager: PMP, Agile, Risk Management"\n3. Each role MUST be on its own line starting with "- "\n4. Each role MUST have a colon ":" followed by skills\n5. Do NOT add any sub-bullets, descriptions, or extra text\n6. Do NOT use bold, italics, or other formatting on role names\n7. If a category has no items, write "- None"\n8. Your output must contain ONLY these four sections with their bulleted lists\n9. The "Required Roles" section is MANDATORY and must contain at least 3 roles for any project`;
    },
...
12. **Milestones Are MANDATORY**: You MUST create a Milestones section with a properly formatted Markdown table. This is REQUIRED for every project.
13. **No Commentary**: Output ONLY "## WBS", "## Tasks", and "## Milestones" sections.
...
## Milestones
CRITICAL: You MUST create a Milestones table. This is MANDATORY and REQUIRED for every project.
...
The table MUST include at minimum: Project Start, Preliminary Design Review Complete, Sprint 1 Complete, Sprint 2 Complete (and all other sprints), Critical Design Review Complete, and Project Completion.
`;
    },
